...
from app.config import EmailSettings, ProjectSettings
...
def reset_password_email_content(
    message, email, password_reset_token
) -> MIMEMultipart:
    # Create the plain-text and HTML version of your message
    link = (
        ProjectSettings.SERVER_HOST
        + "/reset_password/?code="
...
def send_reset_password_email(emails: List[str], password_reset_token: str):
    ...
    message = reset_password_email_content(
        message, emails[0], password_reset_token
    )
