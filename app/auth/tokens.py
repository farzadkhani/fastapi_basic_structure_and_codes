...
class AccessToken:
    ...
    def __init__(self):
        ...
        with open(
            get_root() + "auth" + os.sep + "jwtRS256_private.pem", "rb"
        ) as fh:
            self.__signing_key = jwk_from_pem(fh.read())
        with open(
            get_root() + "auth" + os.sep + "jwtRS256_public.pem", "rb"
        ) as fh:
    ...
    def create_access_token(
        ...
        encoded_jwt = self.__instance.encode(
            to_encode, self.__signing_key, self.__algorithm
        )
        return encoded_jwt

    def decode_access_token(self, *, token: str) -> Dict:
        """Decode Access Token"""
        return self.__instance.decode(
            token, self.__verifying_key, do_time_check=False
        )
